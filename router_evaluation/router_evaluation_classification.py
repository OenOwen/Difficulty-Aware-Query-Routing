...
SLM_VAL_LATENCY = 3.1007189569935947
LLM_VAL_LATENCY = 23.806206206190154
LLM_VAL_INPUT_TOKENS = 154.00091508052708
LLM_VAL_OUTPUT_TOKENS = 1897.1786237188874
...
ROUTER_PATH = "trained_routers/trained_svm_classifier.joblib"
USE_UTILITY = False
MODEL_THRESHOLD = 0.5
...
def save_confusion_matrix_figure(conf, label):
    ...
    ax.set_title(f"Routing Confusion Matrix - {label}")
...
def save_p_value_histogram(probs, label):
    ...
    out_dir = Path("figures")
    out_dir.mkdir(parents=True, exist_ok=True)
    filename = f"classification_router_p_values_histogram_{_slugify_dataset_name(label)}.png"
    out_path = out_dir / filename
