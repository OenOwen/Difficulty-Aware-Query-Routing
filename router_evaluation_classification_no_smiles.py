...
SLM_VAL_LATENCY = 2.997632443039878
LLM_VAL_LATENCY = 11.746656273613679
LLM_VAL_INPUT_TOKENS = 119.61035724176128
LLM_VAL_OUTPUT_TOKENS = 1090.7227914705068
...
ROUTER_PATH = "trained_routers/trained_no_smiles_svm_classifier.joblib"
USE_UTILITY = False
MODEL_THRESHOLD = 0.5
...
print(f"Routing threshold p*: {THRESHOLD:.4f}  (route to LLM when P(complex|x) >= p*)")
...
def save_confusion_matrix_figure(conf, label):
    ...
    filename = f"routing_confusion_matrix_{_slugify_dataset_name(label)}.png"
    ...
    ax.set_title(f"Routing Confusion Matrix on Classifier (No Smiles) - {label}")
...
def save_p_value_histogram(probs, label):
    """Save histogram of trained router p-values for a dataset."""
    out_dir = Path("figures")
    out_dir.mkdir(parents=True, exist_ok=True)
    filename = f"classification_no_smiles_router_p_values_histogram_{_slugify_dataset_name(label)}.png"
    out_path = out_dir / filename

    fig, ax = plt.subplots(figsize=(7, 5))
    ax.hist(probs, bins=20, range=(0.0, 1.0), color="#4c78a8", edgecolor="white")
    ax.set_xlabel("p")
    ax.set_ylabel("Count")
...
def evaluate_dataset(df, probs, label):
    ...
    missed_mask = (routing_df["gt_route"] == "llm") & (routing_df["router_route"] == "slm")
...
for label, slm_file, llm_file in datasets:
    ...
    probs = router.predict_proba(embeddings)[:, 1]

    evaluate_dataset(df, probs, label)
    save_p_value_histogram(probs, label)
