...
def compare_answers(model_response, correct_answer):
    ...
    try:
        result = _verify_in_process(extracted, correct_answer)
        if result:
            return True
    except Exception as e:
...
def main():
    ...
    for i, (_, row) in enumerate(df.iterrows()):
        result, extracted_answer, ground_truth, is_correct, time_taken, unique_id = process_problem(row)
    ...
    save_results_to_csv(results, f"dataset/MATH_results/Qwens/math_evaluation_results_{model_used.replace('/', '_')}.csv")
    print(f"\nModel: {model_used}")
