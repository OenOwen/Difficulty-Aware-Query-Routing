...
SLM_VAL_LATENCY = 2.9976
LLM_VAL_LATENCY = 11.7467
LLM_VAL_INPUT_TOKENS = 119.6104
LLM_VAL_OUTPUT_TOKENS = 1090.7228
...
ROUTER_PATH = "trained_routers/trained_svr_regressor_nosmiles.joblib"
USE_UTILITY = False
MODEL_THRESHOLD = 0.6
...
def save_p_value_histogram(scores, label):
    ...
    out_dir = Path("figures")
    out_dir.mkdir(parents=True, exist_ok=True)
    filename = f"regression_no_smiles_router_p_values_histogram_{_slugify_dataset_name(label)}.png"
    out_path = out_dir / filename
...
datasets = [
    ...
    ("MMLU-Pro (Without Smiles)", MMLU_SLM_FILE, MMLU_LLM_FILE),
]
